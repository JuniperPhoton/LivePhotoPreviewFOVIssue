...
actor CaptureService {
    ...
    private let delegate = DelegateImpl()
...
class DelegateImpl: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    func captureOutput(
        ...
        // If you:
        // 1. Set the videoZoomFactor of the current device to some valuels like 1.2, 1.5
        // 2. Set photoOutput.isLivePhotoCaptureEnabled to true
        // 3. Running this demo on iOS 18.3 (or iOS 18.3.1)
        //
        // Then the image we got here will have different FOV than the one in `AVCaptureVideoPreviewLayer`
        //
        // To preview the image, the quickest way is using Xcode's Core Image debug funcaionality:
        // just hover the cursor to the image variable, and then click the eye icon.
        print("got the preview image")
    }
